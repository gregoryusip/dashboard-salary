...
export default {
  ...
  computed: {
    chartMaritalStatusData() {
      ...
      let married = this.rows.filter((res) => res.marital === 'Sudah menikah')
      return {
        labels: ['Not Married', 'Married'],
        datasets: [
          {
            label: ['Not Married', 'Married'],
            data: [notMarried.length, married.length],
            backgroundColor: ['#8bb0b9', '#b2d0d3'],
            borderColor: ['#8bb0b9', '#b2d0d3'],
            borderWidth: 2,
          },
        ],
      }
    ...
    chartEducationFormalData() {
      ...
      let D3 = this.rows.filter((res) => res.education.includes('D3'))
      let S1 = this.rows.filter((res) => res.education.includes('S1'))
      let S2 = this.rows.filter((res) => res.education.includes('S2'))
      let S3 = this.rows.filter((res) => res.education.includes('D3'))
      return {
